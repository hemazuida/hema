...
public class SerialPort {
	...
	public int write_bytes(byte[] buffer){
		if(!isOpen()){
			logE("Serial port is not open");
			return -1;
		}
		...
		int res=NativeFunc.write(mFd, buffer, buffer.length);
		return res;
	}
	...
	public int write_string(String buffer){
		if(!isOpen()){
			logE("Serial port is not open");
			return -1;
		}
		...
		try {
			byte[] byts=buffer.getBytes("GBK");
			return write_bytes(byts);
